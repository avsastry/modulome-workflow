...
def main(results_dir,outdir):
    ...
    for filename in os.listdir(results_dir):
        if filename.endswith('_cds.txt'):
            sample_id = filename[:filename.find('_cds.txt')]
            transcripts = pd.read_csv(os.path.join(results_dir,filename),sep='\t',skiprows=1)
...
if __name__ == '__main__':
    # Argument parsing
    p = argparse.ArgumentParser(description='Combines featureCount files into one TPM file')
    p.add_argument('-d','--results-dir', help='Directory containing all featureCount results')
